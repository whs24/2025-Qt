...
Sniper::Sniper(QGraphicsItem* parent)
    : Gun(2, 800, parent) // 2发子弹，射击间隔800ms
{
    loadWeaponPixmaps();
}

void Sniper::loadWeaponPixmaps() {
    weaponPixmap = QPixmap(":/Items/weapons/big.png");
    if (weaponPixmap.isNull()) {
        qDebug() << "Sniper: ERROR - weaponPixmap is null!";
    }
}
...
void Sniper::shoot(bool facingRight) {
    ...
    SniperBullet* bullet = new SniperBullet(startPos, facingRight, getDamage(), owner);
...
void Sniper::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget) {
    Q_UNUSED(option)
    Q_UNUSED(widget)
    ...
    if (!facingRight) {
        ...
        painter->drawPixmap(-70, 0, scaledPixmap);
    }
    // 绘制碰撞箱
